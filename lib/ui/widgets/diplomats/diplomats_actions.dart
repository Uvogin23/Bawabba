...
class DiplomatsActions extends StatefulWidget {
  ...
  @override
  State<DiplomatsActions> createState() => _DiplomatsActions();
}

class _DiplomatsActions extends State<DiplomatsActions> {
  ...
  Future<void> fetchFilteredDilomats(
      BuildContext context, Map<String, dynamic> updatedData) async {
    final url = Uri.parse('${Config.baseUrl}/api/diplomats/filter');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        _showSearchResultsDialog(context, diplomats);
      } else {
...
// Helper function to show the search results dialog
  void _showSearchResultsDialog(
      BuildContext context, List<Diplomat> diplomats) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: diplomats.isEmpty
              ? const Center(
                  child: Text(
                    "لا يوجد دبلوماسيون يطابقون معايير البحث",
                    textAlign: TextAlign.center,
                  ),
                )
              : SingleChildScrollView(
                  ...
                  child: DataTable(
                    ...
                    rows: diplomats.map((diplomats) {
                      return DataRow(
                        color: WidgetStateProperty.all(Colors.transparent),
                        cells: [
                          DataCell(SelectableText(diplomats.id.toString())),
                          DataCell(SelectableText(diplomats.firstName)),
                          DataCell(SelectableText(diplomats.lastName)),
                          DataCell(SelectableText(diplomats.nationality)),
                          DataCell(SelectableText(
                              formatDate(diplomats.arrivalDate))),
                          DataCell(SelectableText(
                              formatDate(diplomats.expectedDepartureDate))),
                          DataCell(SelectableText(diplomats.msgRef)),
                          DataCell(SelectableText(diplomats.receivingAgency)),
                          DataCell(
                            Row(
                              children: [
                                IconButton(
                                  ...
                                  onPressed: () =>
                                      viewDiplomat(diplomats, context),
                                ),
                              ],
                            ),
                          ),
            ...
            ElevatedButton(
              onPressed: () async {
                if (diplomats.isNotEmpty) {
                  await _printDataTable(diplomats); // Pass employees list here
                } else {
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () {
                          ...
                          try {
                            fetchFilteredDilomats(context, updatedData2);
                          } catch (e) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
...
Future<void> _printDataTable(List<Diplomat> diplomats) async {
  ...
  final dataRows = diplomats.map((diplomats) {
    return [
      diplomats.msgRef,
      formatDate(diplomats.expectedDepartureDate),
      diplomats.receivingAgency,
      diplomats.passportNumber,
      diplomats.arrivalFlightInfo,
      formatDate(diplomats.arrivalDate),
      diplomats.nationality,
      diplomats.lastName,
      diplomats.firstName,
      diplomats.id.toString(),
    ];
  }).toList();
