...
Future<List<Diplomat>> fetchHistoryDiplomats() async {
  final response =
      await http.get(Uri.parse('${Config.baseUrl}/api/diplomats/history'));
...
class DiplomatHistory extends StatefulWidget {
  const DiplomatHistory({Key? key}) : super(key: key);

  @override
  State<DiplomatHistory> createState() => _DiplomatHistory();
}

class _DiplomatHistory extends State<DiplomatHistory> {
  ...
  void initState() {
    ...
    super.initState();
    diplomatsFuture = fetchHistoryDiplomats();
    diplomats = [];
  }
