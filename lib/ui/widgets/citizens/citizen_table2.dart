...
Future<List<Citizen>> fetchExpectedCitizens() async {
  final response = await http
      .get(Uri.parse('${Config.baseUrl}/api/citizens/supposedToEnter'));
...
class CitizensTable2 extends StatefulWidget {
  const CitizensTable2({Key? key}) : super(key: key);

  @override
  State<CitizensTable2> createState() => _CitizensTable2();
}

class _CitizensTable2 extends State<CitizensTable2> {
  ...
  void initState() {
    ...
    super.initState();
    citizensFuture = fetchExpectedCitizens();
    citizens = [];
  }
    ...
    return FutureBuilder<List<Citizen>>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          ...
          return const Center(
              child: Text("لا يوجد مواطنين تجاوزوا المدة القانونية"));
        } else {
                ...
                const Row(
                  ...
                  children: [
                    ...
                    Padding(
                      ...
                      child: Text(
                        'المواطنون الذين تجاوزوا المدة القانونية',
