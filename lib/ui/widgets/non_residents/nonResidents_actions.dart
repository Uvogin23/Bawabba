...
class NonResidentsActions extends StatefulWidget {
  ...
  @override
  State<NonResidentsActions> createState() => _NonResidentsActions();
}

class _NonResidentsActions extends State<NonResidentsActions> {
  ...
  final TextEditingController _filterlastNameController =
      TextEditingController();
  final TextEditingController _filterPurposeOfVisitController =
      TextEditingController();
  final TextEditingController _filterHostController = TextEditingController();
  ...
  void _clearForm() {
    ...
    _filterPurposeOfVisitController.clear();
    _filterHostController.clear();
  ...
  Future<void> fetchFilterednonResidents(
    ...
    final url = Uri.parse('${Config.baseUrl}/api/non_residents/filter');
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        List<NonResident> list = data
            .map<NonResident>((item) => NonResident.fromJson(item))
            .toList();
        nonResidents = list;
        // Show a dialog based on the results
        _showSearchResultsDialog(context, nonResidents);
      } else {
...
// Helper function to show the search results dialog
  void _showSearchResultsDialog(
      BuildContext context, List<NonResident> tourists) {
    showDialog(
      context: context,
      builder: (BuildContext context) {
        return AlertDialog(
          ...
          content: tourists.isEmpty
              ? const Center(
                  child: Text(
                    "لا يوجد رعايا يطابقون معايير البحث",
                    textAlign: TextAlign.center,
                  ),
                )
                  ...
                  Row(
                    ...
                    children: [
                      ...
                      ElevatedButton(
                        onPressed: () {
                          final updatedData2 = {
                            if (_filterPurposeOfVisitController.text.isNotEmpty)
                              'purpose_of_visit':
                                  _filterPurposeOfVisitController.text,
                            if (_filterHostController.text.isNotEmpty)
                              'host': _filterHostController.text,
                          ...
                          try {
                            fetchFilterednonResidents(context, updatedData2);
                          } catch (e) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              SnackBar(
                                ...
                                ElevatedButton(
                                  ...
                                  onPressed: () {
                                    if (_formKey2.currentState!.validate()) {
                                      ...
                                      showNonResidentsByMonth(context, year!);
                                    }
                                  },
                                  child: const Text(
                                    'بحث',
